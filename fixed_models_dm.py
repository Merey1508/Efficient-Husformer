...
class HUSFORMERModel(nn.Module):
    def __init__(self, hyp_params):
        ...
        self.orig_d_m1, self.orig_d_m2, self.orig_d_m3,self.orig_d_m4,self.orig_d_m5,self.orig_d_m6  = hyp_params.orig_d_m1, hyp_params.orig_d_m2, hyp_params.orig_d_m3,hyp_params.orig_d_m4,hyp_params.orig_d_m5, hyp_params.orig_d_m6
        self.d_m = hyp_params.d_m
        self.c_dm = hyp_params.c_dm
        self.s_dm = hyp_params.s_dm
        self.c_heads = hyp_params.c_heads
        self.c_layers = hyp_params.c_layers
        self.s_heads = hyp_params.s_heads
        self.s_layers = hyp_params.s_layers
        ...
        self.alpha = hyp_params.alpha

        combined_dim = self.d_m     
        output_dim = hyp_params.output_dim       
        self.channels = hyp_params.m1_len+hyp_params.m2_len+hyp_params.m3_len+hyp_params.m4_len+hyp_params.m5_len+hyp_params.m6_len
        ...
        self.trans_m1_all = self.get_network(self_type='m1_all', layers=self.c_layers, heads=self.c_heads, d_m=self.c_dm)
        self.trans_m2_all = self.get_network(self_type='m2_all', layers=self.c_layers, heads=self.c_heads, d_m=self.c_dm)
        self.trans_m3_all = self.get_network(self_type='m3_all', layers=self.c_layers, heads=self.c_heads, d_m=self.c_dm)
        self.trans_m4_all = self.get_network(self_type='m4_all', layers=self.c_layers, heads=self.c_heads, d_m=self.c_dm)
        self.trans_m5_all = self.get_network(self_type='m5_all', layers=self.c_layers, heads=self.c_heads, d_m=self.c_dm)
        self.trans_m6_all = self.get_network(self_type='m6_all', layers=self.c_layers, heads=self.c_heads, d_m=self.c_dm)
        
        # 3. Self Attentions
        self.trans_final = self.get_network(self_type='policy', layers=self.s_layers, heads=self.s_heads, d_m=self.s_dm)
        
        # 4. Projection layers
        self.proj1 = self.proj2 = nn.Linear(combined_dim, combined_dim)
        self.out_layer = nn.Linear(combined_dim, output_dim)

    def get_network(self, self_type='l', layers=-1, heads=-1, d_m=-1):
    
        return TransformerEncoder(embed_dim=d_m,
                                  num_heads=heads,
                                  layers=layers,
    ...
    def forward(self,m1,m2,m3,m4,m5, m6):
        ...
        proj_all = torch.cat([proj_x_m1 , proj_x_m2 , proj_x_m3 , proj_x_m4 , proj_x_m5, proj_x_m6], dim=0)
        ...
        last_hs1 = torch.cat([m1_with_all, m2_with_all, m3_with_all, m4_with_all, m5_with_all, m6_with_all] , dim = 0)
