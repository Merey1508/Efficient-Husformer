    ...
    def train(model, optimizer, criterion):
        ...
        for i_batch, (batch_X, batch_Y, batch_META) in enumerate(train_loader):
            ...
            if batch_chunk > 1:
                ...
                eval_attr = eval_attr.squeeze().float().unsqueeze(1)  # BCEWithLogitsLoss expects float labels
                # Ensure correct shape and integer type
                # Convert to (batch_size,) and integer type
                #eval_attr = eval_attr.float().unsqueeze(1)  # Convert targets to shape [batch_size, 1]
                #preds = preds.squeeze(1)  # Ensure preds has shape [batch_size]
                raw_loss = criterion(preds, eval_attr)
                combined_loss = raw_loss 

                combined_loss.backward()
                preds_class = (torch.sigmoid(preds) > 0.5).float()  # Convert logits to binary class labels
                # Get predicted class (1024,)
    ...
    def evaluate(model, criterion, test=False):
        ...
        with torch.no_grad():
            for i_batch, (batch_X, batch_Y, batch_META) in enumerate(loader):
                ...
                eval_attr = torch.where(eval_attr == 1, torch.tensor(1), eval_attr)   # 1 → 1
                #eval_attr = torch.where(eval_attr == 2, torch.tensor(2), eval_attr)   # 2 → 2
                eval_attr = eval_attr.squeeze().float().unsqueeze(1)  # Ensure correct shape and integer type
        ...
        cm = confusion_matrix(truths4, results3, labels=[0, 1])

        # Compute class-wise accuracy
        class_accuracies = []
        for i in range(2):  # Three classes: 0, 1, 2
            TP = cm[i, i]
            FN = sum(cm[i, :]) - TP  # FN = actual positives - correctly predicted ones
            acc_i = TP / (TP + FN) if (TP + FN) > 0 else 0  # Avoid division by zero
            print(f'Class-{i}: acc_{i} = {acc_i}, sum_{i} = {sum(cm[i, :])}')
            class_accuracies.append(acc_i)

        acc = sum(class_accuracies) / 2  # Average over three classes
        f1 = f1_score(truths4, results3, average='macro')  # Macro-averaged F1-score

        print(f"Evaluation {'Test' if test else 'Validation'} - Loss: {avg_loss:.4f}, MAE: {mae:.4f}, Acc: {acc:.4f}, F1: {f1:.4f}")
        
        return avg_loss, results1, truths2, mae
