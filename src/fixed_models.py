...
class HUSFORMERModel(nn.Module):
    def __init__(self, hyp_params):
        ...
        self.trans_m1_all = self.get_network(self_type='m1_all', layers=self.c_layers, heads=self.c_heads)
        self.trans_m2_all = self.get_network(self_type='m2_all', layers=self.c_layers, heads=self.c_heads)
        self.trans_m3_all = self.get_network(self_type='m3_all', layers=self.c_layers, heads=self.c_heads)
        self.trans_m4_all = self.get_network(self_type='m4_all', layers=self.c_layers, heads=self.c_heads)
        self.trans_m5_all = self.get_network(self_type='m5_all', layers=self.c_layers, heads=self.c_heads)
        self.trans_m6_all = self.get_network(self_type='m6_all', layers=self.c_layers, heads=self.c_heads)
        
        # 3. Self Attentions
        self.trans_final = self.get_network(self_type='policy', layers=self.s_layers, heads=self.s_heads)
    ...
    def get_network(self, self_type='l', layers=-1, heads=-1, d_m=-1):
        if self_type in ['m1_all','m2_all','m3_all','m4_all','m5_all', 'm6_all', 'policy']:
            embed_dim, attn_dropout = self.d_m, self.attn_dropout
            print('Self type is in list')
        else:
